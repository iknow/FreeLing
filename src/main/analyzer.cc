...
//---------------------------------------------
void PrintTree(parse_tree::iterator n, int depth, const config &cfg) {
	...
	if (cfg.OutputFormat==MORFO || cfg.OutputFormat==TAGGED ) {
	  ...
	  for(ait=w->selected_begin(); ait!=w->selected_end(); ait++){
		...
		par=par+"+"+r->get_parole();
	      ...
	      cout<<" "<<lem.substr(1)<<" "<<par.substr(1)<<" "<<ait->get_prob();
	    }
	    else {
	      cout<<" "<<ait->get_lemma()<<" "<<ait->get_parole()<<" "<<ait->get_prob();
	    }
	    if (cfg.SENSE_SenseAnnotation!=NONE) OutputSenses(*ait,cfg);
	  }
	}
	cout<<endl;
...
void ProcessPlain(const config &cfg, tokenizer *tk, splitter *sp, maco *morfo, POS_tagger *tagger, nec* neclass, senses* sens, chart_parser *parser, dependencyMaker *dep) {
  ...
  list<sentence> ls;
    ...
    while (std::getline(std::cin,text)) {
      ...
      if (cfg.OutputFormat>=TOKEN) av=tk->tokenize(text,offs);
      if (cfg.OutputFormat>=SPLITTED) ls=sp->split(av, cfg.AlwaysFlush);
      if (cfg.OutputFormat>=MORFO) morfo->analyze(ls);
      if (cfg.OutputFormat>=MORFO && cfg.SENSE_SenseAnnotation!=NONE) sens->analyze(ls);
      if (cfg.OutputFormat>=TAGGED) tagger->analyze(ls);
      if (cfg.OutputFormat>=TAGGED && cfg.NEC_NEClassification) neclass->analyze(ls);
      if (cfg.OutputFormat>=PARSED) parser->analyze(ls);
      if (cfg.OutputFormat>=DEP) dep->analyze(ls);
    ...
    if (cfg.OutputFormat>=SPLITTED) ls=sp->split(av, true);  //flush splitter buffer
    if (cfg.OutputFormat>=MORFO) morfo->analyze(ls);
    if (cfg.OutputFormat>=MORFO && cfg.SENSE_SenseAnnotation!=NONE) sens->analyze(ls);
    if (cfg.OutputFormat>=TAGGED) tagger->analyze(ls);
    if (cfg.OutputFormat>=TAGGED && cfg.NEC_NEClassification) neclass->analyze(ls);
...
void ProcessToken(const config &cfg, splitter *sp, maco *morfo, POS_tagger *tagger, nec *neclass, senses *sens, chart_parser *parser, dependencyMaker *dep) {
  ...
  unsigned long totlen=0;
    ...
    while (std::getline(std::cin,text)) {
      // get next word
      word w(text); 
      w.set_span(totlen,totlen+text.size()); totlen+=text.size()+1;
      av.push_back(w);
...
void ProcessSplitted(const config &cfg, maco *morfo, POS_tagger *tagger, nec *neclass, senses *sens, chart_parser *parser, dependencyMaker *dep) {
  ...
  unsigned long totlen=0;
	...
	sin>>form; 	
        ...
        // build new word
        word w(form);
	w.set_span(totlen,totlen+form.size()); totlen+=text.size()+1;
	...
	if (cfg.InputFormat<MORFO && cfg.OutputFormat>=MORFO) morfo->analyze(ls);
	if (cfg.InputFormat<SENSE && cfg.OutputFormat>=MORFO && cfg.SENSE_SenseAnnotation!=NONE) sens->analyze(ls);
	if (cfg.InputFormat<TAGGED && cfg.OutputFormat>=TAGGED) tagger->analyze(ls);
	if (cfg.InputFormat<=TAGGED && cfg.OutputFormat>=TAGGED && cfg.NEC_NEClassification) neclass->analyze(ls);
	if (cfg.InputFormat<PARSED && cfg.OutputFormat>=PARSED) parser->analyze(ls);
        if (cfg.InputFormat<DEP && cfg.OutputFormat>=DEP) dep->analyze(ls);
...
int main(int argc, char **argv) {
  ...
  if (!((cfg.InputFormat<cfg.OutputFormat) ||
        (cfg.InputFormat==cfg.OutputFormat && cfg.InputFormat==TAGGED && cfg.NEC_NEClassification))) {
  ...
  if (cfg.InputFormat<SENSE && cfg.OutputFormat>=MORFO && cfg.SENSE_SenseAnnotation!=NONE) {
  ...
  // NEC requested
  if (cfg.InputFormat<=TAGGED && cfg.OutputFormat>=TAGGED && cfg.NEC_NEClassification) {
  ...
  // Input is plain text.
  if (cfg.InputFormat == PLAIN)  ProcessPlain(cfg,tk,sp,morfo,tagger,neclass,sens,parser,dep);
  // Input is tokenized. 
