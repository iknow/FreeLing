...
namespace ukb {
  ...
  namespace prank {
	...
	size_t init_out_coefs(const G & g,
	  ...
	  boost::tuples::tie(v, end) = vertices(g);
	  size_t N = 0;
	  for (; v != end; ++v) {
		size_t i = 0;
		typename graph_traits<G>::out_edge_iterator e, e_end;
		boost::tuples::tie(e, e_end) = out_edges(*v, g);
		float total_w = 0.0;
		for(; e != e_end; ++e) {
		  ++i;
		  total_w += wmap[*e];
		...
		if (i) {
		  ...
		  boost::tuples::tie(ie, ie_end) = in_edges(*v, g);
		  if (ie == ie_end) {
			W[*v] = -1.0;
		  } else {
			W[*v] = 0.0;
	...
	float update_pRank(G & g,
	  ...
	  for (; v_it != end; ++v_it) {
		...
		boost::tuples::tie(e, e_end) = in_edges(v, g);
		for(; e != e_end; ++e) {
	...
	void init_degree(const G & g,
	  ...
	  boost::tuples::tie(v, v_end) = vertices(g);
	  for(; v != v_end; ++v) {
		In[*v] = out_degree(*v, g);
	  }
	}
