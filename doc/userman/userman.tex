...
\label{cap-analyzer}
 ...
 to obtain several linguistic processings.
  ...
  processed. Server-client pairs may communicate via named pipes
  or via sockets.
...
\end{verbatim}
  ...
  If \verb#channel-name# is ommited, the analyzer is started in
  straight mode.  If it is a number, it will be assumed to be a port
  and a server will be launched to accept socket connections on that
  port.  Finally, if \verb#channel-name# is alphanumeric, it will be
  assumed to be the name of a named pipe from which the server has to 
  read requests.
...
\subsection{Straight mode}
 ...
 When \verb#channel-name# is ommited, a stand-alone analyzer will be
...
\subsection{Named pipe client/server mode}
  ...
  If \verb#channel-name# is specified and is an alphanumeric string
  (any valid filename is acceptable), a server is initiated, and named
  pipes with the given name are created.\\ E.g.:
\begin{verbatim}
 analyze mychannel -f en.cfg  &
\end{verbatim}
...
\end{verbatim}
 ...
 in the output.
  ...
  Note also that for this client-server pair to work, they have to share  
  diskspace, since named pipes are accessible as filenames.


\subsection{Socket client/server mode}

  A client-server version based on sockets is also available.  The
  advantatges are that clients are served in sequential order (that
  is, a client will have to wait to the previous one ending its
  request before it is allowed to send text to the server), and that
  clients do not need to be on the same machine than the server.

  To start the server, proceed as in the named pipe version, but
 using a port number as a channel identifier.  \\ E.g:
\begin{verbatim}
 analyze 54321 -f en.cfg  &
\end{verbatim}
  
  Then, clients can request analysis to the server, with:
\begin{verbatim}
 analyzer_client localhost 54321  <myinput  >myoutput
\end{verbatim}
