...
Nevertheless, FreeLing package provides a quite complete application program ({\tt analyzer})
that enables an end user to obtain the analysis of a text. See section \ref{sec-analyzer} for details.
...
\section{Contributions}
   ...
   package or the {\tt Contributions} section in FreeLing webpage to learn more.
...
\section{Requirements}
 ...
 \item Enough hard disk space (about 350Mb)
 ...
 \item Some external libraries are required to compile FreeLing:
    ...
    \item {\tt libpcre} (version 4.3 or higher)\\ 
     ...
     \item {\tt libdb} (version 4.1.25 or higher) \\
       Berkeley
       DB. Included in all Linux distributions. You probably have it
     ...
     \item {\tt Omlet \& Fries} (omlet v.1.0 or higher, fries v.1.0 or higher)//
        Machine Learning utility libraries, used by Named Entity Classifier.
        Installation scripts are not very clever, so these libraries are 
...
\end{itemize}
   ...
   Note that if you (or the library package) install those libraries
...
\end{itemize}
The last command must be issued as root or with {\tt sudo} privileges.
...
new programs which may require linguistic analysis services. 
  ...
  Nevertheless, a main program is included in the package for
 those who just want a text analyzer. This program may 
 be adapted to fit your needs (e.g. customized input/output formats).
  
  The usage and options of this main program is described in section \ref{sec-analyzer}.
    
  Also, in the directory src/main/simple\_examples in the tarball, you
...
%..................................................
\section{Numbers Detection Module}
\label{file-numb}

 XXXXXXXXXXXXX
 parameters: thousants, decimal.
 language dependent.
...
The file consists of two (optional) sections: \verb#<Suffixes># and \verb#<Prefixes>#. The first one contains suffixation rules, and the second, prefixation rules.
...
\begin{enumerate}
 ...
 \item Lemma to assign: F=original form, R=root found in dict, L=lemma in matching dict entry, AF/AR/AL=affix followed by F/R/L, FA/RA/LA=F/R/L followed by affix.
 \item Try the affix always, not only for unknown words.
 \item Retokenization info, explained below ("-" if the suffix doesn't cause retokenization).
\end{enumerate}
...
\subsection{Basic NER Options File}
...
\subsection{{\em BIO} NER Options File}
...
\end{tabular}
  ...
  Affix analysis applies known affixation rules to the word to check whether it is a derived form of a known word  (see option Affix Rules File, below).
...
\end{tabular}
   ...
   This is closely related to affix analysis, see section \ref{file-suf} for details.
...
AffixFile=/usr/local/share/FreeLing/es/sufixos.dat\\
