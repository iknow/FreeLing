...
They have been tested in Ubuntu (8.04 Hardy, 8.10 Intrepid, 9.04 Jaunty) and Debian (5.0.2 Lenny).

The installation procedure for these distributions (will probably work for other debian-based distros) is the following:
...
In Ubuntu Intrepid, Ubuntu Jaunty, and Debian Lenny, installing this single package, will pull in all the required others:
\begin{verbatim}
...
\end{list}
  ...
  build C++ support using the {\tt --enable-cxx} option with {\tt
    ./configure}.
...
\subsubsection{Format for {\tt .src} dictionary files}
  ...
  Whitespaces act as separators, so, make sure not to have extra
