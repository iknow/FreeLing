...
\vspace*{7cm}
\begin{center}
{\Large FreeLing User Manual\\[1ex]\large 2.2}\\
\vspace*{1cm}
{\small September 2010}\\
\end{center}
\end{titlepage}
...
end-user application (e.g. anything from a machine translation system
to a syntactic-tree drawing interface) can access those data and
...
This program offers a set of options that cover almost all FreeLing
...
from your own application program as described above.

%..................................................
\section{Supported Languages}

The distributed version supports (to different extents) English,
Spanish, Catalan, Galician, Italian, Portuguese, Asturian and Welsch.

See the {\tt Linguistic Data} section on FreeLing webpage to find out
about the size of the morphological dictionaries and the source of
their data.
...
\begin{itemize}
   ...
   \item Spanish sense dictionary is a reduced subset extracted from EuroWordNet, 
        This subset, included in this FreeLing package, is distributed under GPL.

   \item Catalan sense dictionary is extracted from CREL project, funded by the
      Catalan government, and is distributed under GPL.
...
\noindent The linguistic data are licensed under diverse licenses,
depending on their original sources.
...
\section{Contributions}
  ...
  Center at Universitat Politecnica de Catalunya (http://talp.upc.edu). 
  ...
  A detailed list can be found in {\sl Contributions} section at FreeLing webpage \\
  ({\tt http://www.lsi.upc.edu/\~{ }nlp/freeling}).
...
\begin{list}{\labelitemi}{\leftmargin=1em}
 ...
 \item Some external libraries are required to compile FreeLing:
    ...
    \item {\tt libpcre} (version 4.3 or higher)\\ 
      ...
      \itemsep 0cm
      \item Ubuntu/Debian: {\tt libpcre3-dev libpcre3}
      \item OpenSuse/Fedora/Mandriva: {\tt libpcre libpcre-devel}
      \item Slackware: {\tt pcre}
      \end{list}
     ...
     \item {\tt libdb} (version 4.1.25 or higher) \\
      ...
      \begin{list}{\labelitemiii}{\leftmargin=1em}
      \itemsep 0cm
      \item Ubuntu/Debian: {\tt libdb4.7 libdb4.7-dev libdb4.7++ libdb4.7++-dev}
      \item OpenSuse/Fedora/Mandriva: {\tt libdb-4.7 libdb-4.7-devel libdb\_cxx-4.7}\\
                                      {\tt libdb\_cxx-4.7-devel }
      \item Slackware: {\tt db4}
      \end{list}
     ...
     \item {\tt libboost} (version 1.31 or higher) \\
      ...
      \begin{list}{\labelitemiii}{\leftmargin=1em}
       \itemsep 0cm
       \item Ubuntu/Debian: {\tt libboost-dev libboost-filesystem libboost-program-options}
       \item OpenSuse/Fedora/Mandriva: {\tt libboost-devel libboost }
       \item Slackware: {\tt boost }
       \end{list}

     \item {\tt Omlet \& Fries} (omlet v.1.0.1 or higher, fries v.1.2 or higher)\\
        ...
        Already included in FreeLing binary {\tt .deb} packages.
...
They have been tested in Ubuntu (8.04 Hardy, 8.10 Intrepid, 9.04 Jaunty, 9.10 Karmic, 10.04 Lucid) and Debian (5.0.5 Lenny).
...
\end{verbatim}
  ...
  Libraries {\tt libfries}, and {\tt libomlet} are included in the FreeLing 
  {\tt .deb} package, so you do not need to install them.
...
the usual \verb#./configure && make && make install# stuff).
...
\end{verbatim}
  ...
  In other distributions, check the distribution package manager to install a working C++ compiler.
...
Just open your favorite software package manager and install them.
  ...
  If you have another distribution, use your package manager to locate and install
  the appropriate library packages (see section \ref{sec-requirements}).
...
\end{list}
  ...
  {\bf NOTE: } Alternatively, you can install the libraries above from
  source. {\sl Don't} do that unless you know what you're
  ...
  libraries. All of them use the standard {\tt ./configure \&\& make \&\&
    make install} procedure. In the case of BerkeleyDB, you have to
  build C++ support using the {\tt --enable-cxx} option with {\tt ./configure}.
...
\end{verbatim}
 ...
 Omlet\&Fries libraries are contained in the files {\tt libomlet.so} and {\tt libfries.so}, installed by default in {\tt /usr/local/lib} (or {\tt /usr/lib} if installed from binary package). If you are a developer, you need to know that {\tt libfries.so} contains the classes storing linguistic information, which are described in section \ref{ssec-data}.
...
effort (additional pcre/db libraries porting might be 
required also...). 
...
\end{verbatim}
  ...
  The created object will create the required submodules, and when asked to {\tt analyze}
...
\begin{itemize}
 ...
 \item The dictionary file name. This may be a BerkeleyDB--indexed file (with extension {\tt .db}) or either a plain text file (with extension {\tt .src}). See below for details.
...
\subsection{Form Dictionary File}
  ...
  The form dictionary is either a plain text file, or a
  BerkeleyDB--indexed file. 
  The dictionary module relies on the
  extension to decide which format to expect ({\tt .src} for plain
  text, {\tt .db} for indexed files)

  The plain text dictionary file ({\tt .src}) format is described below.
  This file can be directly passed to the constructor of the dictionary search module.

  Indexed Berkeley-DB files ({\tt .db}) may be created with the {\tt indexdict} 
  program provided with FreeLing, which is called with the command:
  \begin{verbatim}
   indexdict indexed-dict-name.db  <source-dict.src 
  \end{verbatim}
  Where {\tt source-dict.src} is a plain text dictionary, and {\tt
  indexed-dict-name.db} is the resulting indexed file, which can be
  directly passed to the constructor of the dictionary search module.

  See the (very simple) source code in {\tt src/main/utilities/indexdict.cc}
  if you're interested on how it is indexed.

\subsubsection{Format for {\tt .src} dictionary files}

  Each line in the file must have the lemma--PoS list for one word.
  That is, each line has the format: {\tt form lemma1 PoS1 lemma2 PoS2 ...}. \\ 
  E.g.:\\ 
  \verb#  casa casa NCFS000 casar VMIP3S0 casar VMM02S0#\\
  \verb#  backs back NNS back VBZ#

  \noindent Whitespaces act as separators, so, make sure not to have extra
  whitespaces between fields or at the end of the line.
  \smallskip
  ...
  \noindent This line expresses that whenever the form {\sl del} is found, it is
  ...
  \smallskip
   ...
   \noindent Nevertheless, a set of tags may be specified for a given form, e.g.:\\
  \verb#  he'd he+'d PRP+VB/MD#

   \noindent This will produce two words: {\sl he} with {\tt PRP} analysis, and
...
\end{verbatim}
  ...
  \item Section \verb#<Affixes># contains a list of words that may be part of a NE --either prefixing or suffixing it-- even if they are lowercased.
  For instance, this is the case of the word {\sl don} in Spanish (e.g. {\sl don\_Juan} should be a NE, even if {\sl don} is lowercased), or the word {\sl junior} or {\sl jr.} in English (e.g. {\sl Peter\_Grasswick\_jr.} should be a NE, even if {\sl jr.} is lowercased).
 
  The section should containt a word per line, followed by the keyword {\tt PRE} or {\tt SUF} stating whether the word may be attached before or after an NE. It a word should be either a prefix or a suffix, it must be declared in two different lines, one with each keyword.
\begin{verbatim}
<Affixes>
don  PRE
doña PRE
...
%..................................................
\section{Ortographic Correction Module}
\label{file-orto}
  ...
  For instance, if a mispelled word such as {\em cavallo} is found, this module will find out that it sounds the same than a correct word in the dictionary ({\em caballo}), thus providing analysis for that otherwise unknown word.
...
\begin{itemize}
  \item The file name of the semantic relationship graph to load. This
    ...
    \verb#data/common/wnet30-rels.txt# and  \verb#data/common/wnet30g-rels.txt# 
...
\begin{verbatim}
   ...
   compile_ukb -o $FLSHARE/common/xwn30-ukb.bin $FLSHARE/common/wnet30-rels.txt $FLSHARE/common/wnet30g-rels.txt
\end{verbatim}
    (\verb#$FLSHARE# refers to the \verb#share/FreeLing# directory in
your FreeLing installation, which defaults to
\verb#/usr/local/share/FreeLing# if you installed from source 
...
\end{verbatim}
    ...
    if you installed from source, or \verb#/usr/share/FreeLing# if you 
    used a binary {\tt .deb} package.
...
\label{sec-pos}
  ...
  The {\tt hmm\_tagger} module is somewhat faster than {\tt relax\_tagger}, but the later allows you to add manual constraints to the model. Its API is the following:
\begin{verbatim}
...
\end{itemize}
  ...
  The {\tt relax\_tagger} module can be tuned with hand written constraint, but is about 2 times slower than {\tt hmm\_tagger}.
\begin{verbatim}
...
\label{file-hmm}
   ...
   The file may be generated by your own means, or using a
    tagged corpus and the script {\tt src/utilitities/TRAIN} provided
    in FreeLing package. See comments in the script file to find out
    which format the corpus is expected to have.

  The file has seven sections: \verb#<Tag>#, \verb#<Bigram>#,
  ...
  (estimated via your preferred method).  Each line is a tag probability {\tt P(t)} with format \\
  {\tt Tag Probability}
 ...
 {\tt InitialState LogProbability}.
  ...
  ($b_{iw}$ parameters of the HMM).
   ...
   \noindent will assign zero probability to any sequence containing the verb ``haber'' tagged 
...
\end{verbatim}
     ...
     programs that can be found in {\tt src/utilities/coref}, which
     use the libomlet package. (see {\tt
...
\end{verbatim}
 ...
 {\tt g++ -o sample sample.cc -lmorfo -ldb\_cxx -lpcre -lomlet -fries -lboost\_filesystem}
...
\label{cap-analyzer}
 ...
 to obtain several linguistic processings.
  ...
  FreeLing also provides a couple of programs {\tt analyzer\_server}
  ...
  processed. Server-client pairs communicate via sockets.

  Both {\tt analyzer} and {\tt analyzer\_server} are usually called 
  with an option \verb#-f config-file#  (if ommitted, they will search
  ...
  The {\tt analyze} script described below handles all these default
  paths and variables and makes everything easier if you want to use
  the defaults
 ...
 \section{The easy way: Using the {\tt analyze} script}
  ...
  decide whether you want the client-server or the straight version.
...
\end{verbatim}
  ...
  If \verb#port-number# is ommited, the analyzer is started in
 ...
 directory. 
  ...
  source, and \verb#/usr/share/FreeLing# if you used a binary {\tt
    .deb} package).
...
\subsection{Straight mode}

 When \verb#port-number# is ommited, a stand-alone analyzer will be
 launched.  It will load the configuration, read input from stdin,
...
\subsection{Client/server mode}
  ...
  If \verb#port-number# is specified, a server is initiated and
  starts listening for incoming requests.\\ E.g.:
\begin{verbatim}
 analyze 50005 -f en.cfg  &
\end{verbatim}

\noindent Then, clients can request analysis to the server, with:
\begin{verbatim}
...
\end{verbatim}
(where \verb#xx.xx.xx.xx# should be the IP address or hostname for
the machine where the server was started)

 Any client trying to connect to the server while it is attending another
 request, will be blocked and attended when the current request is finished.
...
lemma-tag for the word (including the selected one).
 ...
 to retrain the taggers with the script {\tt src/utilitities/TRAIN}
 provided in FreeLing package. See comments in the script file for
 details about how to use it.
  
\item {\bf Use UTF8 encoding}

\begin{tabular}{|l|l|}
Command line       & Configuration file   \\ \hline
\verb#--utf# & \verb#UseUTF=(yes|y|on|no|n|off)#  \\ \hline
\end{tabular}

  FreeLing internally uses \verb#latin1# ISO encoding.
 When this option is active, the input is assumed to be in \verb#utf8#, 
and converted to \verb#latin1# before processing. The results
are converted back to \verb#utf8# before being output.

  Note that this will {\em not} work with \verb#utf8# encodings that cannot be
converted to \verb#latin1#.

  When using the server version (see chapter~\ref{cap-analyzer}), you
  can choose whether it will be the server or the client who will
  perform the conversion. 

  Specifying \verb#--utf# at the server command line
  means that the server expects \verb#utf8# input, so the client must 
  send so (thus, no conversion must be carried out by the client).
  
  Specifying \verb#--utf# at the client command line means
  that the client will convert \verb#utf8# input to \verb#latin1# before
  sending the request to the server. So, the server has to be 
  expecting \verb#latin1# input (i.e. no conversion in the server side).
