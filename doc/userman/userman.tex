...
\itemsep 0cm
 ...
 {\tt InitialState LogProbability}.
  ...
  ($b_{iw}$ parameters of the HMM).
   ...
   The first tag may be a wildcard (\verb#*#), which will match any tag.
   The tag \verb#0# may be used to denote sentence beginning.
   ...
   \end{verbatim}
   \noindent will assign zero probability to all sequences containing
   \verb#PT.NC# (noun following an interrogative pronoun), to all 
   sequences containing  \verb#0.DT.VMI# (that is, verb following a 
   determiner at sentence beggining), and to all sequences containing 
   \verb#SP.PP.NC# (noun following a preposition and a personal pronoun).
...
\label{cap-analyzer}
 ...
 to obtain several linguistic processings.
  ...
  For instance, you can use the defalt configuration files, located
  at {\tt /usr/local/share/FreeLing/config}. Note that those
...
\subsection{Named pipe client/server mode}
...
\begin{verbatim}
 analyzer_client xx.xx.xx.xx 54321  <myinput  >myoutput
\end{verbatim}
