...
Installation follows standard GNU autoconfigure installation procedures.
...
\item Install packaged requirements
...
Just open your favorite software package manager and install them.
 ...
 As an example, commands to install the packages from command line in Ubuntu are 
provided, though you can do the same using synaptic or a similar manager.

\noindent {\bf Install libpcre headers:}
\begin{verbatim}
 sudo apt-get install libprec3-dev
\end{verbatim}

\noindent {\bf Install libdb headers}\\
 (version doesn't need to be 4.6. Any 4.x available in your distribution will do):
\begin{verbatim}
 sudo apt-get install libdb4.6++-dev
\end{verbatim}

\noindent {\bf Install libboost headers and binaries}\\
 (in Ubuntu, installing this single package, will pull in all the required others):
\begin{verbatim}
 sudo apt-get install libboost-dev
\end{verbatim}


\item Install non-packaged requirements

\noindent {\bf Install libcfg+ library}\\
...
\item Install FreeLing

\noindent {\bf Install libfries:}
\begin{verbatim}
...
\end{verbatim}
 ...
 FreeLing library is entirely contained in the file {\tt libmorfo.so} installed by default in {\tt /usr/local/lib}.

 Sample program {\tt analyze} is installed in {\tt /usr/local/bin}. See sections \ref{sec-execute} and \ref{cap-analyzer} for details.
...
\label{mod-ukb}
  ...
  \cite{soroa09}, which is integrated in FreeLing and distributed as-is
  ...
  text using PageRank algorithm. See \cite{soroa09} for details on the
  algorithm.
